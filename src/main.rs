...
fn main() -> Result<()> {
    ...
    let mut render_state = pollster::block_on(RenderState::new(
        Arc::clone(&window),
    ...
    event_loop.run(move |event, elwt| match event {
        Event::WindowEvent { window_id, ref event } if window_id == window.id() => match event {
            ...
            WindowEvent::MouseWheel { delta, .. } => {
                ...
                match delta {
                    MouseScrollDelta::LineDelta(_, y) => state.scroll(-(*y as isize) * 3),
                    MouseScrollDelta::PixelDelta(pos) => state.scroll(-(pos.y as isize / 15)),
                }
            }
            ...
            WindowEvent::Resized(physical_size) => {
                ...
                terminal_state.write().unwrap().resize(new_cols, new_rows);
            ...
            WindowEvent::RedrawRequested => match render_state.render() {
                Ok(_) => {}
